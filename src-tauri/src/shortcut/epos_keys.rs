//! Epos Keys based keyboard shortcut implementation
//!
//! This module provides an alternative to Tauri's global-shortcut plugin
//! using the Epos Keys wrapper for more control over keyboard events.
//!
//! ## Architecture
//!
...
/// State for the Epos Keys shortcut manager
pub struct EposKeysState {
...
impl EposKeysState {
    ...
    fn manager_thread(cmd_rx: Receiver<ManagerCommand>, app: AppHandle) {
        info!("Epos Keys manager thread started");
        ...
        loop {
            ...
            while let Some(event) = manager.try_recv() {
                if let Some((binding_id, hotkey_string)) = hotkey_to_binding.get(&event.id) {
                    debug!(
                        "Epos Keys event: binding={}, hotkey={}, state={:?}",
            ...
            match cmd_rx.recv_timeout(std::time::Duration::from_millis(10)) {
                Ok(cmd) => match cmd {
                    ...
                    ManagerCommand::Shutdown => {
                        info!("Epos Keys manager thread shutting down");
                        break;
                    }
                },
        ...
        info!("Epos Keys manager thread stopped");
    }
    ...
    fn do_unregister(
        ...
        if let Some(id) = binding_to_hotkey.remove(binding_id) {
            ...
            debug!("Unregistered Epos Keys shortcut: {}", binding_id);
        }
        Ok(())
    }
    ...
    pub fn start_recording(&self, app: &AppHandle, binding_id: String) -> Result<(), String> {
        ...
        debug!("Started Epos Keys recording mode");
        Ok(())
    }
    ...
    fn recording_loop(app: AppHandle, running: Arc<AtomicBool>) {
        while running.load(Ordering::SeqCst) {
            let event = {
                let state = match app.try_state::<EposKeysState>() {
                    Some(s) => s,
                    None => break,
                };
            ...
            if let Some(key_event) = event {
                ...
                // Emit to frontend
                if let Err(e) = app.emit("epos-keys-event", &frontend_event) {
    ...
    pub fn stop_recording(&self) -> Result<(), String> {
        ...
        debug!("Stopped Epos Keys recording mode");
        Ok(())
    }
}

impl Drop for EposKeysState {
    fn drop(&mut self) {
...
/// Convert Epos Keys modifiers to a list of strings
...
/// Validate a shortcut string for the Epos Keys implementation.
/// Epos Keys is more permissive: allows modifier-only combos and the fn key.
pub fn validate_shortcut(raw: &str) -> Result<(), String> {
    if raw.trim().is_empty() {
        return Err("Shortcut cannot be empty".into());
    }
    // Epos Keys accepts modifier-only, key-only, and modifier+key combos
    // Just verify the string is parseable
    raw.parse::<Hotkey>()
        .map(|_| ())
        .map_err(|e| format!("Invalid shortcut for Epos Keys: {}", e))
}

/// Initialize Epos Keys shortcuts
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    let state = EposKeysState::new(app.clone())?;
    ...
    for (id, default_binding) in default_bindings {
        ...
        if let Err(e) = state.register(&binding) {
            error!(
                "Failed to register Epos Keys shortcut {} during init: {}",
                id, e
            );
        }
    }
    ...
    app.manage(state);
    info!("Epos Keys shortcuts initialized");
    Ok(())
}
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Some(cancel_binding) = get_settings(&app_clone).bindings.get("cancel").cloned() {
                if let Some(state) = app_clone.try_state::<EposKeysState>() {
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    let state = app
        .try_state::<EposKeysState>()
        .ok_or("EposKeysState not initialized")?;
...
#[tauri::command]
#[specta::specta]
pub fn start_epos_keys_recording(app: AppHandle, binding_id: String) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::EposKeys {
        return Err("Epos Keys is not the active keyboard implementation".into());
    }
...
#[tauri::command]
#[specta::specta]
pub fn stop_epos_keys_recording(app: AppHandle) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::EposKeys {
        return Err("Epos Keys is not the active keyboard implementation".into());
    }
